...
from frappe_agents.runner.run import CONTEXT_CHARS_PER_TOKEN
...
class TestContextLimit(AgentTestCase):
	...
	def seed_turns(self, count: int) -> tuple[str, list[str]]:
		...
		with as_user(RESTRICTED_USER):
			for index in range(count):
				...
				prior = make_run(
					...
					message=f"{mark} question {'q' * TURN_CHARS}",
				)
				prior.flags.ignore_permissions = True
				prior.db_set(
					{"status": "Completed", "output_message": f"{mark} answer {'a' * TURN_CHARS}"},
	...
	def test_a_profile_with_no_limit_still_sends_the_whole_history(self):
